...
ssh "$HEAD_NODE" "cd $PROJ_NAME;source vllm_env_moe;source vllm_env_vars_ray;./start_ray.sh head $HEAD_NODE_IP $NODE_IFACE"
sleep 1

# Connect to the worker nodes and start the ray workers
for WORKER in $WORKER_NODES; do
    echo "Worker node: $WORKER"
    ssh "$WORKER" "cd $PROJ_NAME;source vllm_env_moe;source vllm_env_vars_ray;./start_ray.sh worker $HEAD_NODE_IP $NODE_IFACE"
done
