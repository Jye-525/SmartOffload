...
LOG_BASE_PATH="/lus/grand/projects/VeloC/jye/hipc25_sched_mt/"

TESTA_CASES=("prompt-decode") #  "prompt-only" "decode-only"  "prompt-decode"
NUM_TRIES=10
# NUM_REQS=(5000) # for sharegpt
NUM_REQS=(8) # for multi-news and gov_report 

DATASET_NAME="random" # longbench, gsm8k, sharegpt, random, fixed-len
...
# gpu_mem_utils=(0.9 0.8 0.7) # GPU memory utilization
...
SUBTASK="multi_news" # used for longbench, choices: gov_report, multi_news, lcc (500)
declare -A DATASETS=(
...
# INPUT_LENS=(32767 16383 8191 4095 2047 1023 511 255) # input length, used for synthetic datasets
# INPUT_LENS=(511) # input length, used for synthetic datasets
INPUT_LENS=(511 255 127 63 31) # set bs=4
OUT_LENS=(1 1 1 1 1) # output length, used for synthetic datasets
RANDOM_RATIO=0 
declare -A SYNT_DATASETS=(
    ["random"]="--dataset-name random --random-input-len __INPUT_LEN__ --random-output-len __OUT_LEN__ --random-range-ratio ${RANDOM_RATIO}"
    ["fixed-len"]="--dataset-name fixed-len --fixed-input-len __INPUT_LEN__ --fixed-output-len __OUT_LEN__"
)

declare -A SYNT_DATASETS_OFFLINE=(
    # Format: "DATASET_NAME"
    ["random"]="--dataset-name random --input-len __INPUT_LEN__ --output-len __OUT_LEN__ --random-range-ratio ${RANDOM_RATIO}"
    ["fixed-len"]="--dataset-name fixed-len --fixed-input-len __INPUT_LEN__ --fixed-output-len __OUT_LEN__"
)
...
MAX_NUM_BATCHED_TOKENS=(32768) # max number of tokens in a batch, 512, 1024, 2048, 4096, 8192, 16384, 32768

IS_USE_V1=$([ -z "$VLLM_USE_V1" ] || [ "$VLLM_USE_V1" = "0" ] && echo 0 || echo 1)
MAX_NUM_BATCHED_REQS=256
...
run_offline_bench() {
    ...
    client_cmd="python ${EXEC_PATH}/benchmark_throughput_v2.py --backend vllm \
                    ...
                    --scheduler-cls ${SCHEDULER_CLS} --num-prompts $num_req --async-engine --disable-log-requests \
                    --max-num-seqs ${MAX_NUM_BATCHED_REQS} "
        ...
        for max_num_batched_tokens in ${MAX_NUM_BATCHED_TOKENS[@]}; do
            for num_req in ${NUM_REQS[@]}; do
                ...
                for i in "${!INPUT_LENS[@]}"; do
                    ...
                    # Replace placeholders with actual values
                    dataset_config="${SYNT_DATASETS_OFFLINE[$DATASET_NAME]}"
                    dataset_config="${dataset_config//__INPUT_LEN__/$input_len}"
                    dataset_config="${dataset_config//__OUT_LEN__/$out_len}"
                    echo "Dataset: $DATASET_NAME, Config: $dataset_config"
    ...
    case "$DATASET_NAME" in
        ...
        "random"|"fixed-len")
            if [ $SERVE_TYPE = "online" ]; then
                ...
                # offline benchmark
                benchmark_offline_with_synthetic_dataset $gen_len 
            fi 
            ;;
        *)
