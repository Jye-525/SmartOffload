...
class OffloadBuffer:
    def __init__(self, k: int, param_offload_target:str):
        ...
        self.to_fetch_static_modules = deque() # save the static modules that needs to be fetched from GPU. To be used in reorganize
            ...
            for layer_idx in range(self.start_layer, self.end_layer):
                ...
                if (layer_idx + 1) % self.curr_k_value == 0:
                    ...
                    if next(layer.parameters()).device == torch.device("cpu"):
                        ...
                        if layer_idx + 1 < 2 * self.curr_k_value:
                            ...
                            # put this layer to the to_fetch_static_modules queue
                            self.to_fetch_static_modules.append(layer)
                            # print(f"++++++add {layer_idx} to the to_fetch_static_modules queue", flush=True)
    ...
    def wraper_forward(self, module: torch.nn.Module, args=None, kwargs=None):
        ...
        if len(self.to_fetch_static_modules) > 0:
            top_module = self.to_fetch_static_modules[0]
            idx = self.offloaded_modules[0].layer_idx if len(self.offloaded_modules) > 0 and self.offloaded_modules[0].layer_idx < self.curr_k_value else self.end_layer
            if top_module.layer_idx < idx:
                # print(f"++++++ fetch the layer_id {top_module.layer_idx} to static buffer at layer {cur_layer_idx}", flush=True)
                # fetch the static module from the CPU buffer to the GPU buffer
                self.to_fetch_static_modules.popleft()
                layer = self.layers[top_module.layer_idx - self.start_layer]
                layer_dict = self.per_layer_info[top_module.layer_idx - self.start_layer]
                layer_params = layer_dict['parameters']
                with torch.cuda.stream(self.data_mv_stream):
                    gpu_data = torch.zeros(layer_dict['total_elems'],
                                        dtype=layer_dict['dtype'], device=self.device)
                    ...
                    for name, p in layer.named_parameters():
                        ...
                        gpu_data[start_offset:end_offset].copy_(p.data.view(-1), non_blocking=True)
                        p.data = gpu_data[start_offset:end_offset].view_as(p.data)  
                self.prefetch_events[top_module.layer_idx - self.start_layer].record(self.data_mv_stream)
                self.recording_events[top_module.layer_idx - self.start_layer] = True
            # else:
            #     print(f"==============={top_module.layer_idx} is not less than {idx}, skip the fetch. end_layer={self.end_layer}", flush=True) 
        ...
        with nvtx_range(f"wraper_fwd_step2_{cur_layer_idx}"):
            if self.recording_events[cur_inner_layer_idx]:
                ...
                # print(f"++++++ wraper_fwd_step2: synchronize the prefetch event for layer {cur_layer_idx}", flush=True)
                ...
                if ((self.curr_k_value == 1) or (cur_inner_layer_idx + 1) % self.curr_k_value == 1):
                    ...
                    if len(self.offloaded_modules) > 0:
                        ...
                        if prefetech_layer_idx < self.end_layer:
                            # print(f"++++++ prefetch the layer_id {prefetech_layer_idx} to dynamic buffer at layer {cur_layer_idx}", flush=True)
        ...
        with nvtx_range(f"wraper_fwd_step4_{cur_layer_idx}"):
            # print(f"++++++ run the forward function for layer {cur_layer_idx}", flush=True)
