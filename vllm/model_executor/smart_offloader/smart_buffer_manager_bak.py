...
class LayerManager:
    ...
    def add_tensor(self, name: str, p: Union[torch.nn.Parameter, torch.Tensor], kv_map: Tuple[int, int, str] = None):
        ...
        # is_tensor = torch.is_tensor(p)
        if (not self.on_gpu_static):
            # print(f"++++Layer {self.layer_id} tensor {name}: p.device={p.device} p_ref_counts={sys.getrefcount(p) - 1}", flush=True)
            t = p
            ...
            # print(f"Layer {self.layer_id} tensor {name}: t.device={t.device} p.device={p.device} p_ref_counts={sys.getrefcount(p) - 1}", flush=True)
            p.data = t
        self.tensors[name] = p
        ...
        # is_same_ptr = (self.tensors[name].data_ptr() == self.pinned_cpu_tensors[name].data_ptr())
        # print(f"====Layer {self.layer_id} tensor {name}: tensors[name].device={self.tensors[name].device} pinned_cpu_tensors[name].device={self.pinned_cpu_tensors[name].device} p_ref_counts={sys.getrefcount(p) - 1}, is_tensor={is_tensor}, is_same_ptr={is_same_ptr}, tensor_refs={sys.getrefcount(self.tensors[name]) - 1}, cpu_tensor_refs={sys.getrefcount(self.pinned_cpu_tensors[name]) - 1}", flush=True) 
        if self.layer_id == self.start_layer:
            self.move_to_cuda()
        # print(f"After Add tensor, Layer {self.layer_id} tensor {name}: tensors[name].device={self.tensors[name].device} pinned_cpu_tensors[name].device={self.pinned_cpu_tensors[name].device} p_device={p.device}, p_ref_counts={sys.getrefcount(p) - 1}, tensor_refs={sys.getrefcount(self.tensors[name]) - 1}, cpu_tensor_refs={sys.getrefcount(self.pinned_cpu_tensors[name]) - 1}", flush=True)
    ...
    def move_to_cpu(self):
        ...
        if 'self_attn.qkv_proj.weight' in self.tensors:
            print(f"+++++++Layer {self.layer_id} move_to_cpu. module.device={next(self.module.parameters()).device}, tensors.device={self.tensors['self_attn.qkv_proj.weight'].device}, pinned_tensor.device={self.pinned_cpu_tensors['self_attn.qkv_proj.weight'].device}", flush=True)
    ...
    def map_module_tensors(self, to_device: torch.device):
        if self.module is None:
            return
        print(f"111111111 Before map_module_tensors to_device={to_device}, tensors.device={self.tensors['self_attn.qkv_proj.weight'].device}, pinned_tensor.device={self.pinned_cpu_tensors['self_attn.qkv_proj.weight'].device}, module.device={next(self.module.parameters()).device}", flush=True)
        for name, t in self.module.named_parameters():
            print(f"*************map_module_tensors same_pointer={(t.data_ptr() == self.pinned_cpu_tensors[name].data_ptr())} , {(t.data_ptr() == self.tensors[name].data_ptr())}, {(self.tensors[name].data_ptr() == self.pinned_cpu_tensors[name].data_ptr())}", flush=True)
            t.data = self.tensors[name] if to_device == self.cuda_device else self.pinned_cpu_tensors[name]
        
        print(f"222222 After map_module_tensors to_device={to_device}, tensors.device={self.tensors['self_attn.qkv_proj.weight'].device}, pinned_tensor.device={self.pinned_cpu_tensors['self_attn.qkv_proj.weight'].device}, module.device={next(self.module.parameters()).device}", flush=True)
    ...
    def begin_compute(self):
        ...
        if not self.on_gpu_static:
            ...
            self.varify_offloaded_layer_tensors(self.layer_id, -1)
    ...
    def varify_offloaded_layer_tensors(self, layer_id: int, flag_id: int):
        module_device = next(self.module.parameters()).device
        # Check if the tensor content on the CPU and GPU are the same
        for name, t in self.tensors.items():
            if name == "kv_cache":
                continue
            else:
                print(f"{flag_id}----End Compute---varify layer {layer_id} tensor {name}: t.device = {t.device}, pinned_tensor_device={self.pinned_cpu_tensors[name].device}, module.device={module_device}", flush=True)
                break
...
class SmartBufferManager:
    ...
    def add_tensor(self, layer_id: int, name: str, p: Union[torch.nn.Parameter, torch.Tensor]):
        ...
        if layer_id == 0 or layer_id == 1:
            logger.info(f"Layer {layer_id} tensor {name} shape={p.shape} size={p.data.numel() * p.data.element_size()}")
    ...
    def begin_compute(self, layer_id: int):
        ...
        if layer_id == self.start_layer:
            self.fwd += 1
        self.layers[layer_id].print_per_layer_tensors(self.fwd, layer_id)
        ### end debugging
    ...
    def end_compute(self, layer_id: int):
        ...
        if (not self.layers[layer_id].on_gpu_static) and (self.k_layers is not None):
            ...
            # Verify the layer_id if it is dynamic GPU parameter
            self.layers[layer_id].varify_offloaded_layer_tensors(layer_id, 1)
            
            self.layers[layer_id].move_to_cpu()
            
            self.layers[layer_id].varify_offloaded_layer_tensors(layer_id, 2) 
            ...
            self.layers[next_k_layer].varify_offloaded_layer_tensors(next_k_layer, 3)
        
        # logger.info(f"Memory after performing layer {layer_id} computation. Used memory: {print_mem_stats() / (1024**2):.2f} MB")
        # if self.fwd == 2 and layer_id == self.end_layer - 1:
        #     raise RuntimeError("End of forward pass")
        ...
        for idx, layer_id in enumerate(range(self.start_layer, self.end_layer)):
            if idx % k == 0:
                with nvtx_range(f"SmartBufferManager::move_{layer_id}_to_cpu"):
                    ...
                    self.layers[layer_id].move_to_cpu()
                    print(f"Layer {layer_id} is a dynamic GPU parameter on CPU")
            else:
