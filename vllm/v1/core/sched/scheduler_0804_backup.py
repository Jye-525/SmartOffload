...
class Scheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        self.resume_window_limit = 5 # refers to the checking window for resuming requests to avoid frequent preempt/resume same request
        logger.info(f"Scheduler initialized with method: {self.schedule_method}, kv_cache_blocks={kv_cache_config.num_blocks}")
        ...
        #######
        # Track KV usage history for adaptive threshold
        self.preemption_kv_usage_history: deque[float] = deque(maxlen=100)  # Last 100 preemption events
        self.adaptive_kv_threshold = 1  # Start with default
        # self.min_kv_threshold = 0.85  # Minimum threshold
        # self.max_kv_threshold = 0.98  # Maximum threshold
        self.threshold_update_frequency = 10  # Update every N preemptions

    def schedule(self) -> SchedulerOutput:
        if self.schedule_method == "default":
            return self._schedule_default()
        elif self.schedule_method == "cost-aware":
            return self._schedule_cost_aware()
        elif self.schedule_method == "evict-optimal":
            return self._schedule_evict_optimal()
        elif self.schedule_method == "evict-optimal-2":
            # schedule the resumed resquest based on if there are enough kv cache 
            # for prefill or recomputed tokens of the resumed request 
            return self._schedule_evict_optimal_2()
        elif self.schedule_method == "evict-optimal-3":
            ...
            return self._schedule_evict_optimal_3()
        elif self.schedule_method == "evict-optimal-4":
            # schedule the resumed resquest based on if there are enough kv cache 
            # for prefill or recomputed tokens of the resumed request + running_reqs + reqs_from_waiting_list 
            return self._schedule_evict_optimal_4()
        elif self.schedule_method == "evict-optimal-5":
            return self._schedule_evict_optimal_5()
        elif self.schedule_method == "evict-optimal-5-1":
    ...
    def _schedule_evict_optimal(self) -> SchedulerOutput:
        # NOTE(woosuk) on the scheduling algorithm:
        # There's no "decoding phase" nor "prefill phase" in the scheduler.
        # Each request just has the num_computed_tokens and
        # num_tokens_with_spec. num_tokens_with_spec =
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            while True:
                ...
                if new_blocks is None:
                    ...
                    total_preempted_tokens += preempted_req.num_computed_tokens
                    preempted_req.num_recomputed_tokens = preempted_req.num_computed_tokens
                    # log the preemption request
                    # logger.debug(f"Scheduler preempted request {preempted_req.request_id} with {preempted_req.num_computed_tokens} tokens on step {self.schedule_step_count + 1}," 
                    #             f"raw prompt tokens: {preempted_req.num_prompt_tokens} recomputed tokens: {preempted_req.num_recomputed_tokens} all tokens: {preempted_req.num_tokens}") 
                    preempted_req.status = RequestStatus.PREEMPTED
                    preempted_req.last_preempted_step_id = self.schedule_step_count
                    preempted_req.repeat_preempt_count += 1
                    preempted_req.preempt_step_ids.append(self.schedule_step_count)
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                # Method 2: based on the number of scheduled running requests
                if request.status == RequestStatus.PREEMPTED and cur_scheduled_running_reqs >= self.num_last_scheduled_running_reqs:
                    # If there are not request exit compared with last iteration, the request will be died again if resumed 
                    break
    ...
    def _schedule_evict_optimal_2(self) -> SchedulerOutput:
        # NOTE(woosuk) on the scheduling algorithm:
        # There's no "decoding phase" nor "prefill phase" in the scheduler.
        # Each request just has the num_computed_tokens and
        # num_tokens_with_spec. num_tokens_with_spec =
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                # check if the resumed request can be scheduled
                # Method 3: based on if the available KV cache have enough room for the prefill/recomputed tokens
                if request.status == RequestStatus.PREEMPTED:
                    req_tokens = max(request.num_prompt_tokens, request.num_recomputed_tokens)
                    # Check If there are not request exit compared with last iteration, the request will be died again if resumed 
                    num_required_blocks = cdiv(req_tokens, self.kv_cache_manager.block_size)
    ...
    def _schedule_evict_optimal_3(self) -> SchedulerOutput:
        # NOTE(woosuk) on the scheduling algorithm:
        # There's no "decoding phase" nor "prefill phase" in the scheduler.
        # Each request just has the num_computed_tokens and
        # num_tokens_with_spec. num_tokens_with_spec =
    ...
    def _schedule_evict_optimal_4(self) -> SchedulerOutput:
        # NOTE(woosuk) on the scheduling algorithm:
        # There's no "decoding phase" nor "prefill phase" in the scheduler.
        # Each request just has the num_computed_tokens and
        # num_tokens_with_spec. num_tokens_with_spec =
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                # check if the resumed request can be scheduled
                # Method 4: based on if the available KV cache have enough room for the prefill/recomputed tokens of current request
                extra_tokens = len(scheduled_new_reqs) + len(scheduled_resumed_reqs) + len(scheduled_running_reqs)
                if request.status == RequestStatus.PREEMPTED:
                    tmp_req_tokens = max(request.num_prompt_tokens, request.num_recomputed_tokens) + extra_tokens 
                else:
                    tmp_req_tokens = request.num_prompt_tokens + extra_tokens
                # Check If there are not request exit compared with last iteration, the request will be died again if resumed 
                num_required_blocks = cdiv(tmp_req_tokens, self.kv_cache_manager.block_size)
                if num_required_blocks > self.kv_cache_manager.block_pool.get_num_free_blocks():
                    break
            ...
            while self.waiting and token_budget > 0:
                ...
                if request.status == RequestStatus.PREEMPTED:
                    if self.num_last_scheduled_running_reqs <= cur_scheduled_running_reqs:
                        ...
                        # no request is exit
                        break
                    # est_output_tokens = max(request.num_prompt_tokens, request.num_recomputed_tokens) - request.num_prompt_tokens
                    # est_output_tokens = max(est_output_tokens, self.avg_output_len) 
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            while True:
                ...
                if new_blocks is None:
                    ...
                    # Preempt the lowest-priority request. (remove the most recently last added request)
                    preempted_req = self.running.pop()
                    self._record_preemption_kv_usage()
                    self.kv_cache_manager.free(preempted_req)
                    total_preempted_tokens += preempted_req.num_computed_tokens
                    preempted_req.num_recomputed_tokens = preempted_req.num_computed_tokens
                    preempted_req.status = RequestStatus.PREEMPTED
    ...
    def _schedule_cost_aware(self) -> SchedulerOutput:
        # NOTE(woosuk) on the scheduling algorithm:
        # There's no "decoding phase" nor "prefill phase" in the scheduler.
        # Each request just has the num_computed_tokens and
        # num_tokens_with_spec. num_tokens_with_spec =
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                # check if the resumed request can be scheduled
                ## if self.kv_cache_manager.usage >= 0.8 and request.status == RequestStatus.PREEMPTED:
                # Method 1: based on the KV cache usage and resume window limit
                if self.kv_cache_manager.usage >= 0.5 and request.status == RequestStatus.PREEMPTED:
                    # Under heavy KV cache usage scenario, check if the preempted requests are resumed in a short window.
                    # if yes, this request cannot be scheduled and wait for the next step.
                    if ((self.schedule_step_count - request.last_preempted_step_id) < self.resume_window_limit or 
                        (request.last_try_resume_step_id > 0 and 
                         self.schedule_step_count - request.last_try_resume_step_id < self.resume_window_limit)):
                        request.last_try_resume_step_id = self.schedule_step_count
                        break
    ...
    def _record_preemption_kv_usage(self):
        # record the current KV usage when detecting a preemption occurs
        current_kv_usage = self.kv_cache_manager.usage
        self.preemption_kv_usage_history.append(current_kv_usage)
        
        self._update_adptive_kv_threshold()
            
    def _update_adptive_kv_threshold(self):
        # Update the adaptive KV threshold based on the history of preemption events
        
        # calculate the min usage and avg usage from the window of preemption events
        slide_window_size = 20
        recent_usage = list(self.preemption_kv_usage_history)[-slide_window_size:] 
        min_recent_usage = min(recent_usage)
        # avg_recent_usage = sum(recent_usage) / slide_window_size
        
        # set the adaptive threhold to min_recent_usage
        self.adaptive_kv_threshold = min_recent_usage
